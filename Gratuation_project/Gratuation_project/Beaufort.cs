...
{
    public partial class Beaufort : Form
    {
        public Beaufort()
        {
            InitializeComponent();
        ...
        private void Beaufort_Load(object sender, EventArgs e)
        {
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("The key must only contain letters.", "Input Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            string encryptedText = BeaufortCipherEncrypt(plainText, key);
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            ...
            string decryptedText = BeaufortCipherDecrypt(cipherText, key);
            textBox3.Text = decryptedText;
        }
        private string BeaufortCipherEncrypt(string text, string key)
        {
            text = text.ToUpper();
            key = key.ToUpper();
            ...
            for (int i = 0; i < text.Length; i++)
            {
                ...
                {
                    result.Append(textChar); // Non-alphabetic characters remain unchanged
                    continue;
                }

                char keyChar = key[i % keyLength];
                int cipherChar = (keyChar - char.ToUpper(textChar) + 26) % 26; // Beaufort formula
                result.Append(char.IsLower(textChar)
                    ? (char)('a' + cipherChar) // Preserve lowercase
                    : (char)('A' + cipherChar)); // Preserve uppercase
            }

            return result.ToString();
        }

        // Beaufort Cipher Decryption Method
        // Note: The Beaufort Cipher is symmetric; the encryption and decryption processes are identical.
        private string BeaufortCipherDecrypt(string text, string key)
        {
            return BeaufortCipherEncrypt(text, key);
        }
    }
}
